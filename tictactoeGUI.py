...
def new_game():
    ...
    label.config(text=f"new game.{player} turn")

    for row in range(3):
        for column in range(3):
            buttons[row][column].config(text="", bg="lime")
...
label = Label(text= player + " turn", font=("Century Gothic", 42, "bold"))
label.pack(side="top")

reset_button = Button(text="RESET", font=("Century Gothic", 32), background="purple", command=new_game)
...
for row in range(3):
    for column in range(3):
        buttons[row][column] = Button(frame, text="", font=("Century Gothic", 32), width=7, height=3, background="lime",
